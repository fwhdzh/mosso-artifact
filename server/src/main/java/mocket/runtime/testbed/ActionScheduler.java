...
public class ActionScheduler {
    ...
    private boolean isNoActionWaiting() {
        ...
        for (Request req : waitingList) {
            if (req.requestType == 0) {
                if (currentTransition.action.compare(getAction(req, sid))) {
                    ...
                    waitingLists.get(sid).remove(req);
                    stepNext();
                    return; // If the action is waiting, we should not call the waitAction in the end of stepNext.
                }
            }
        }
        /**
    ...
    private void waitAction(int sid, ActionType type, long actionTimeout) {
        ...
        if (actionTimer != null) {
            throw new RuntimeException("WaitAction is called when there is another action waiting!");
        }
    ...
    private void waitStateChecking(int sid, ActionType type, long stateTimeout) {
        ...
        if (stateTimer != null) {
            throw new RuntimeException("WaitStateChecking is called when there is another state checking waiting!");
        }
