...
public class State implements mocket.path.State {
    ...
    public State updateState(int sid, String[] stateValues) {
        ...
        for (String stateField : stateValues) {
            ...
            } else if (varName.equals("msg")) {
                String[] msgContent = varValue.split("_");
                String type = msgContent[0];
                int sourceNode = Integer.parseInt(msgContent[1]);
                int destinationNode = Integer.parseInt(msgContent[2]);
                int term = Integer.parseInt(msgContent[3]);
                Message m;
                if (msgContent.length == 5) {
                    boolean granted = Boolean.parseBoolean(msgContent[4]);
                    m = new Message(type, sourceNode, destinationNode, term, granted);
                } else if (msgContent.length == 4) {
                    m = new Message(type, sourceNode, destinationNode, term);
                } else {
                    // Undefined messages
                    m = null;
                }
