...
public class Interceptor {
    ...
    private static HashMap<Integer, String[]> sentMsgs = new HashMap<>();
    private static HashMap<Integer, String[]> receivedMsgs = new HashMap<>();
    ...
    private static boolean hasToBeSentMsg(int actionId) {
        return sentMsgs.keySet().contains(actionId);
    }

    private static boolean hasToBeReceivedMsg(int actionId) {
        return receivedMsgs.keySet().contains(actionId);
    }
     ...
     * @return
     */
    public static void getMsg(int actionId, String... params){
        if (actionId > actionCount) {
            // Wrong condition. getMsg is added before collectParams
        } else if (semaphore.keySet().contains(actionId)) {
            // The message is got before notifyAndBlock.
            // It is a message receiving action
            String[] currentParams = Interceptor.params.get(actionId);
            String[] newParams = Arrays.copyOf(currentParams, currentParams.length + params.length);
            for(int i = 0; i < params.length; i++) {
                newParams[i+currentParams.length] = params[i];
            }
            // Synchronize message element with Mocket by notifyAndBlock
            Interceptor.params.put(actionId, newParams);
        } else if (!semaphore.keySet().contains(actionId)) {
            // The message is got after notifyAndBlock.
            // It is a message sending action
            Interceptor.sentMsgs.put(actionId, params);
            // Synchronize message element with Mocket by checkState
        }
    }
    ...
    /**
     * Use after collectParams
     */
    public static void getMsg(String... params){
    ...
    public static boolean checkState(String action, int actionId) {
        ...
        int itemSize = state.stateNames().size();
        if (hasToBeSentMsg(actionId)) {
            itemSize += 1;
        }
        if (hasToBeReceivedMsg(actionId)) {
            itemSize += 1;
        }
        messageBuffer.putInt(itemSize); // States
        ...
        if (hasToBeSentMsg(actionId)) {
            String stateDetail = "msg,";
            String[] content = sentMsgs.get(actionId);
            for (int i = 0; i < content.length; i++) {
                stateDetail += content[i] + "_";
            }
            byte[] contentBytes = stateDetail.getBytes();
            int len = contentBytes.length;
            messageBuffer.putInt(len);
            messageBuffer.put(contentBytes);
            stateString = stateString + "[" + stateDetail.substring(0, stateDetail.length() - 1) + "]";
        }
        if (hasToBeReceivedMsg(actionId)) {
            String stateDetail = "consumedMsg,";
            String[] content = receivedMsgs.get(actionId);
            for (int i = 0; i < content.length; i++) {
                stateDetail += content[i] + "_";
            }
            byte[] contentBytes = stateDetail.getBytes();
