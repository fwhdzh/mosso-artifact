...
public class RaftNode {
    ...
    public void init() {
        ...
        // start election
        Interceptor.checkInitState();
        resetElectionTimer();
    }

    // client set command
    @MocketAction("ClientRequest")
...
//            raftLog.updateMetaData(currentTerm, null, raftLog.getFirstLogIndex());
     ...
     */
    @MocketAction("Timeout")
    private void startVote() {
        Interceptor.collectParams(Integer.toString(localServer.getServerId()));
        lock.lock();
        try {
            if (!ConfigurationUtils.containsServer(configuration, localServer.getServerId())) {
                resetElectionTimer();
    ...
    private class PreVoteResponseCallback implements RpcCallback<RaftProto.VoteResponse> {
        ...
        @MocketAction("HandleRequestVoteResponse")
        @Override
